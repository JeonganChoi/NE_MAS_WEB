    ...
    if cboBank != '' and cboAccount == '':
        ...
        with connection.cursor() as cursor:
            cursor.execute(" SELECT IFNULL(UN.ACDATE, ''), IFNULL(UN.ACIOGB, ''), IFNULL(UN.RESNAM, ''), IFNULL(UN.OACAMTS, 0), IFNULL(UN.IACAMTS, 0) "
                           ...
                           "        , IFNULL(UN.ACDESC, ''), IFNULL(UN.GBN, ''), IFNULL(UN.RESNAM2, ''), IFNULL(UN.ACUSE, ''), IFNULL(UN.ACINFO, '') "
                           " FROM( "
                           ...
                           "                , IFNULL(A.ACDESC, '') AS ACDESC, IFNULL(A.GBN, '') AS GBN, IFNULL(F.RESNAM, '') AS RESNAM2, IFNULL(A.ACUSE, '') AS ACUSE, IFNULL(A.ACINFO, '') AS ACINFO "
                           "         FROM ACTSTMENT A "
                           ...
                           "                , IFNULL(A.ACDESC, ''), IFNULL(A.GBN, ''), IFNULL(F.RESNAM, ''), IFNULL(A.ACUSE, '') AS ACUSE, IFNULL(A.ACINFO, '') AS ACINFO "
                           "         FROM ACTSTMENT A "
                           "         LEFT OUTER JOIN OSREFCP B "
                           "         ON A.ACIOGB = B.RESKEY "
                           "         AND B.RECODE = 'OUA' "
