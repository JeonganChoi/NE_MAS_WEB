    ...
    if all(temp):
        with connection.cursor() as cursor:
            cursor.execute(" SELECT IFNULL(SUM(BAL), 0), IFNULL(SUM(INTOTAL), 0), IFNULL(SUM(OUTTOTAL), 0), IFNULL(SUM(BAL + INTOTAL - OUTTOTAL), 0) FROM "
                           ...
                           "        WHERE ACIOGB = '2' AND ACCUST = '" + str(cust) + "' AND ACACNUMBER = '" + str(act) + "' AND FIN_OPT = 'Y' "
                           ...
                           "        WHERE ACIOGB = '1' AND ACCUST = '" + str(cust) + "' AND ACACNUMBER = '" + str(act) + "' AND FIN_OPT = 'Y' "
                           "        AND ACDATE BETWEEN '" + str(strDate) + "' AND '" + str(endDate) + "' AND ICUST = '" + str(iCust) + "' "
                           " ) AA ")
        ...
        with connection.cursor() as cursor:
            cursor.execute("  SELECT IFNULL(ACIOGB, ''), IFNULL(ACDATE, ''), IFNULL(IN_ACAMTS, 0), IFNULL(OUT_ACAMTS, 0), IFNULL(ACCUST, ''), IFNULL(CUST_NME, ''), IFNULL(ACACNUMBER, ''), IFNULL(ACNUM_NAME, '') FROM "
                           ...
                           "     WHERE A.ACIOGB = '2' AND FIN_OPT = 'Y' "
                           ...
                           "     LEFT OUTER JOIN ACNUMBER C "
                           "     ON A.ACACNUMBER = C.ACNUMBER "
                           "     WHERE A.ACIOGB = '1' AND FIN_OPT = 'Y' "
                           "     AND A.ICUST = '" + str(iCust) + "' "
                           " ) AA "
    ...
    if any(temp):
        with connection.cursor() as cursor:
            cursor.execute(" SELECT IFNULL(SUM(BAL), 0), IFNULL(SUM(INTOTAL), 0), IFNULL(SUM(OUTTOTAL), 0), IFNULL(SUM(BAL + INTOTAL - OUTTOTAL), 0) FROM "
                           ...
                           " UNION ALL "
                           " SELECT 0 AS BAL, SUM(IFNULL(ACAMTS, 0)) AS INTOTAL, 0 AS OUTTOTAL FROM SISACCTT "
                           "        WHERE ACIOGB = '2' AND ACDATE BETWEEN '" + str(strDate) + "' AND '" + str(endDate) + "' AND ICUST = '" + str(iCust) + "' AND FIN_OPT = 'Y' "
                           "        AND ACCUST = '" + str(cust) + "' OR ACACNUMBER = '" + str(act) + "'  "
                           " UNION ALL "
                           " SELECT 0 AS BAL, 0 AS INTOTAL, SUM(IFNULL(ACAMTS, 0)) AS OUTTOTAL FROM SISACCTT "
                           "        WHERE ACIOGB = '1' AND ACDATE BETWEEN '" + str(strDate) + "' AND '" + str(endDate) + "' AND ICUST = '" + str(iCust) + "' AND FIN_OPT = 'Y' "
                           "        AND ACCUST = '" + str(cust) + "' OR ACACNUMBER = '" + str(act) + "' "
                           " ) AA ")
            balresult = cursor.fetchall()
        ...
        with connection.cursor() as cursor:
            cursor.execute(" SELECT IFNULL(SUM(BAL), 0), IFNULL(SUM(INTOTAL), 0), IFNULL(SUM(OUTTOTAL), 0), IFNULL(SUM(BAL + INTOTAL - OUTTOTAL), 0) FROM "
                           " (SELECT SUM(IFNULL(ACAMTS, 0)) AS BAL, 0 AS INTOTAL, 0 AS OUTTOTAL FROM ACBALANCE WHERE ACDATE < '" + str(strDate) + "' AND ICUST = '" + str(iCust) + "' "
                           " UNION ALL "
                           " SELECT 0 AS BAL, SUM(IFNULL(ACAMTS, 0)) AS INTOTAL, 0 AS OUTTOTAL FROM SISACCTT WHERE ACIOGB = '2' AND ACDATE < '" + str(strDate) + "' AND ICUST = '" + str(iCust) + "' AND FIN_OPT = 'Y' "
                           " UNION ALL "
                           " SELECT 0 AS BAL, 0 AS INTOTAL, SUM(IFNULL(ACAMTS, 0)) AS OUTTOTAL FROM SISACCTT WHERE ACIOGB = '1' AND ACDATE < '" + str(strDate) + "' AND ICUST = '" + str(iCust) + "' AND FIN_OPT = 'Y' "
                           " ) AA ")
            balresult = cursor.fetchall()
